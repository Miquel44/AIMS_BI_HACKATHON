...
def api_send_response():
    try:
        ...
        # Print de confirmación en consola
        print("=" * 80)
        print("JSON RECIBIDO CORRECTAMENTE")
        print("=" * 80)
        print(f"Patient ID: {patient_id}")
        print(f"Probabilidad: {result.get('probability', 'N/A')}")
        print(f"Diagnóstico: {result.get('diagnosis', 'N/A')}")
        print("=" * 80)
        ...
        pdf_filename = f"reports/reporte_ckd_{patient_id}_{timestamp}.pdf"
        ...
        if available_features:
            ...
            if completeness < 0.5:
                ...
                story.append(Paragraph(
                    f"<b>ATENCIÓN:</b> Solo {available_count}/{total_possible} características principales están disponibles. "
                    f"El modelo puede tener en cuenta hasta {total_possible} características para una evaluación más precisa.",
                    warning_style
                ))
        ...
        try:
            ...
            with open(pdf_filename, 'rb') as pdf_file:
                ...
                if response.status_code == 200:
                    print(f"✓ PDF enviado exitosamente al sistema médico: {upload_url}")
                else:
                    print(f"⚠ Error al enviar PDF: {response.status_code} - {response.text}")

        except Exception as upload_error:
            print(f"⚠ No se pudo enviar el PDF al sistema médico: {upload_error}")
